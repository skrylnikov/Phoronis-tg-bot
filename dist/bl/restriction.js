...
import { sendMessage, sendMessageWithDelay, deleteMessage, restrictUser } from './actions.js';
export const config = {
    activateList: [['бан'], ['забань']],
};
const { PorterStemmerRu } = natural;
const getResult = (value) => Array.isArray(value) ? value : [value];
const getRestrictTime = (tokenList) => {
...
const getUserName = (user) => pipe(user.userName, O.fold(() => `[Неопознаный космонавт](tg://user?id=${user.id})`, (v) => '@' + v));
export const execute = async ({ replyUser, canRestrictMembers, username, messageId, normalizedTokenList }) => pipe(replyUser, E.fromOption(() => 'Ой, кажется я не смогла найти кого тут нужно забанить :('), E.chain((user) => canRestrictMembers ? E.right(user) : E.left('Кажется ты не можешь банить участников :(')), E.chain((user) => !user.isAdmin ? E.right(user) : E.left('Ой, я не могу банить админов :(')), E.chain((user) => pipe(getRestrictTime(normalizedTokenList), E.map((value) => [
...
]))), E.mapLeft((v) => sendMessage(v, { reply: true })), E.fold(getResult, getResult));

...
export const execute = async ({ normalizedTokenList }) => {
    console.log('Weather');
    const url = `https://suggestions.dadata.ru/suggestions/api/4_1/rs/suggest/address`;
    const cityResponse = await got.post(url, {
        body: JSON.stringify({
    ...
    const findedCity = cityResponse.body.suggestions.find((x) => x.data?.city);
    ...
    console.log(city);
    const weatherResponse = await got.get(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&appid=${openWeatherToken}&lang=ru&units=metric`, { responseType: 'json' });

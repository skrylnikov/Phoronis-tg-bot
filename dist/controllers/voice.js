...
export const voiceController = async (ctx) => {
    try {
        ...
        if (file.length < 1024 * 1024 && duration < 30) {
            const result = await got.post('https://stt.api.cloud.yandex.net/speech/v1/stt:recognize', {
                body: file,
                headers: {
            ...
            const task = await got.post('https://transcribe.api.cloud.yandex.net/speech/stt/v2/longRunningRecognize', {
                json: {
                    config: {
                        specification: {
                            languageCode: 'ru-RU',
            ...
            let counter = 0;
            while (!result.done) {
                const { body } = await got.get(`https://operation.api.cloud.yandex.net/operations/${id}`, {
                    headers: {
                        'Authorization': `Api-Key ${yandexCloudToken}`
                    },
                    responseType: 'json',
            ...
            const text = result?.response?.chunks?.map(({ alternatives }) => alternatives?.[0]?.text).join('. ');
            if (!text) {

...
const rawConfig = [Weather, About, Ping, Restriction, Translate, AI];

const executorList = rawConfig.map((c) => ({
  ...c,
  config: {
    ...c.config,
    activateTokenList: c.config.activateList.map((x) =>
      x.map(PorterStemmerRu.stem)
    ),
  },
}));
...
export const processTextMessage = async (ctx: Context) => {
  ...
  const comand = executorList.find((x) =>
    x.config.activateTokenList.some((x) =>
      x.every((y) => normalizedTokenList.includes(y))
    )
  );
  ...
  if (ctx.chat) {
    try {
      ...
      if (replyUser) {
        replyUser.isAdmin = adminList
          .map((x) => x.user.id)
          .includes(replyUser.id);
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const [result] = await Promise.all([
    comand.execute({
      tokenList,
      normalizedTokenList: without(
        flatten(comand.config.activateTokenList),
        normalizedTokenList
      ),
    ...
    switch (action.type) {
      case Actions.sendMessage: {
        ctx.reply(action.payload, {
          reply_to_message_id: action.meta?.reply
      ...
      case Actions.deleteMessage: {
        ctx.api.deleteMessage(ctx.chatId!, action.payload);
        return;
      }

...
Точная vector query SHOULD сохраняться без ANN-индекса, пока её database p95 не превышает 100 мс на целевом benchmark-профиле. Если budget превышен, система MUST добавить минимально достаточный pgvector-индекс, повторно измерить latency и проверить корректность обязательных фильтров и recall относительно exact baseline.
